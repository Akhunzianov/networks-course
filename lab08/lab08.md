...
![img](s2.png)
![img](s3.png)
Выше видно что передача завершилась успешно, а на скрине ниже видно что отправленный и полученный файлы не отличаются
![img](s4.png)
...
#### Демонстрация работы
В этом задании сервер работает так: сначала принимает файл от клиента, а когда прием закончился отсылает свой файл клиенту. Запускаем сервер:
![img](s5.png)
Запускаем клиент, видим по логам что передача прошла а также файл отправленный клиентом совпадает с полученным сервером и отправленный сервером совпадает с полученным клиентом.
![img](s6.png)
Замечание: в этом задинии снизил симулируемую потерю покетов до 10% чтобы лог не получался слишком большой и чтобы работало быстрее.
...
#### Демонстрация работы
Функции реализованы в файле lib.rs (checksum и verify_checksum), также в этом файле есть тесты. В предыдущее задание контрольные суммы тоже добавлены
![img](s7.png)
